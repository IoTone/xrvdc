...
- **Snap Spectacles** — *別ルート。* Spectacles は汎用 WebXR ブラウザを持たず、Lens Studio の Lens を実行します。Spectacles 向けには Lens Studio を使用してください — [Spectacles & Lens Studio スターター](/staging/news/snap-spectacles-lens-studio-入門/) を参照。なお A-Frame の同一コードベースは、同じ URL で Quest 3 + Vision Pro をカバーします。
...
- Spectacles 向けはこちら：[Snap Spectacles と Lens Studio](/staging/news/snap-spectacles-lens-studio-入門/)
